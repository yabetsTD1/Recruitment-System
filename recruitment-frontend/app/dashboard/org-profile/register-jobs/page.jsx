...
export default function RegisterJobsPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [j, t] = await Promise.all([
        api.get("/admin/registered-jobs"),
        api.get("/admin/job-types"),
      ]);
      setJobs(Array.isArray(j.data) ? j.data : []);
      setJobTypes(Array.isArray(t.data) ? t.data : []);
    } catch (err) {
      ...
      if (status === 401) {
        ...
        setMsg({
          type: "error",
          text: "You don’t have permission to view registered jobs (ADMIN/SUPER_ADMIN only).",
        });
      } else if (status === 404) {
        setMsg({
          type: "error",
          text: "API endpoint not found. Check NEXT_PUBLIC_API_URL and backend routes.",
        });
      } else {
        setMsg({
          type: "error",
          text: "Failed to load jobs. Please check backend is running.",
        });
      }
      setJobs([]);
      setJobTypes([]);
      // Keep console info for debugging
      console.error("RegisterJobsPage.fetchAll failed", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.jobTypeId) {
      setMsg({ type: "error", text: "Please select a Job." });
      return;
    }
    if (!form.classCode) {
      setMsg({ type: "error", text: "Please select a Class." });
      return;
    }
    setSaving(true);
    try {
      ...
      if (status === 401)
        ...
        setMsg({
          type: "error",
          text: "You don’t have permission to register jobs.",
        });
      else setMsg({ type: "error", text: backendMsg || "Failed to save." });
      console.error("RegisterJobsPage.handleSubmit failed", err);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (item) => {
    ...
    try {
      ...
      if (status === 401)
        ...
        setMsg({
          type: "error",
          text: "You don’t have permission to delete jobs.",
        });
      else setMsg({ type: "error", text: backendMsg || "Failed to delete." });
      console.error("RegisterJobsPage.handleDelete failed", err);
    }
  };
    ...
    <div style={{ fontFamily: "sans-serif", color: "#2c3e50" }}>
      ...
      >
        <div>
          ...
          <p
            style={{ color: "#7f8c8d", fontSize: "13px", margin: "4px 0 0 0" }}
          >
            Register jobs by selecting a job type and class
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      >
        {[
          ...
          {
            label: "Job Types Used",
            value: new Set(jobs.map((j) => j.jobTypeId).filter(Boolean)).size,
            color: "#2980b9",
          },
          {
            label: "Classes Used",
      ...
      >
        {loading ? (
          <div
            style={{ padding: "48px", textAlign: "center", color: "#7f8c8d" }}
          >
            Loading...
          </div>
        ) : filtered.length === 0 ? (
          ...
          >
            {jobs.length === 0
              ? 'No jobs registered yet. Click "+ Register Job" to get started.'
                ...
                >
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button
                        onClick={() => goToQualification(item)}
                        style={{
                          padding: "5px 12px",
                          background: "#eaf4fb",
                          color: "#2980b9",
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {editItem ? "Edit Job" : "Register Job"}
              </h2>
              <button
                onClick={() => setShowModal(false)}
                style={{
