...
export default function CertificationsSection({
  ...
  useEffect(() => {
    ...
    if (!email) return;
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/certification?email=${encodeURIComponent(email)}`)
      .then(r => r.json()).then(d => { if (Array.isArray(d)) setSavedRecords(d); }).catch(() => {});
  }, []);

  const deleteSaved = async (id) => {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/certification/${id}`, { method: "DELETE" }).catch(() => {});
  ...
  const removeCertification = (index) => {
    ...
    if (entry.id) {
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/certification/${entry.id}`, { method: "DELETE" }).catch(() => {});
    }
  ...
  const saveEntry = async (index) => {
    ...
    const entry = certifications[index];
    if (!entry.professionalLicense?.trim()) {
      setSaveMsg(p => ({ ...p, [index]: "error:License/Certification name is required" }));
      return;
    }
    setSavingIdx(index);
    try {
      const entry = certifications[index];
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/certification`, {
      ...
      if (refreshEmail) {
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/certification?email=${encodeURIComponent(refreshEmail)}`)
          .then(r => r.json()).then(d => { if (Array.isArray(d)) setSavedRecords(d); }).catch(() => {});
      }
    ...
    <div style={{
        ...
        <div className="animate-in">
          {savedRecords.length > 0 && (
            <div style={{ marginBottom: "24px", background: "white", borderRadius: "10px", overflow: "hidden", border: "2px solid #e2e8f0" }}>
              <div style={{ padding: "10px 16px", background: "linear-gradient(135deg, #f59e0b, #d97706)", color: "white", fontSize: "13px", fontWeight: "700" }}>
                Saved Certifications ({savedRecords.length})
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead style={{ background: "#f8fafc" }}>
                  <tr>
                    {["#", "License/Certification", "Institution", "Skills", "Start", "End", "Action"].map(h => (
                ...
                <tbody>
                  {savedRecords.map((r, i) => (
                    <tr key={r.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "9px 12px", fontSize: "13px", fontWeight: "600", color: "#1f2937" }}>{r.professionalLicense || "—"}</td>
                      <td style={{ padding: "9px 12px", fontSize: "13px", color: "#6b7280" }}>{r.institution || "—"}</td>
                      <td style={{ padding: "9px 12px", fontSize: "13px", color: "#6b7280" }}>{r.skills || "—"}</td>
            ...
            <div key={index} className="entry-card" style={{
              ...
              <div style={{ marginTop: "20px", textAlign: "center" }}>
                ...
                <button type="button" onClick={() => saveEntry(index)} disabled={savingIdx === index}
                  style={{ padding: "10px 28px", background: savingIdx === index ? "#94a3b8" : "linear-gradient(135deg, #f59e0b, #d97706)",
                    color: "white", border: "none", borderRadius: "8px", cursor: savingIdx === index ? "not-allowed" : "pointer",
                    fontSize: "14px", fontWeight: "600", boxShadow: "0 2px 8px rgba(245, 158, 11, 0.3)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: "20px", padding: "0 24px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px" }}>
          ...
          <button type="button"
            onClick={() => {
              const hasUnsaved = certifications.some(e => !e.id && e.professionalLicense?.trim());
              if (hasUnsaved) { setNextError("Please save all entries before proceeding."); return; }
              if (savedRecords.length === 0 && certifications.filter(e => e.id).length === 0) {
                setNextError("Please add and save at least one certification entry.");
                return;
              }
              setNextError("");
              goToNextStep();
