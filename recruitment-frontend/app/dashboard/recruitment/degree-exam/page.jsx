...
export default function DegreeExamPage() {
  ...
  const submitScore = (appId) => {
    const s = scores[appId] || {};
    const written = parseFloat(s.written) || 0;
    const oral = parseFloat(s.oral) || 0;
    const total = Math.round((written + oral) / 2);
    const newStatus = total >= 60 ? "SHORTLISTED" : "REJECTED";
    api.put(`/recruitments/applications/${appId}/status`, { status: newStatus }).then(() => {
      setApplications(prev => prev.map(a => a.id === appId ? { ...a, written, oral, total, scored: true } : a));
  ...
  return (
    <div>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ color: "#6b7280", marginTop: "4px" }}>Enter written and oral exam scores for shortlisted candidates</p>
      </div>
      ...
      {!selectedRec ? (
        <div style={{ background: "white", borderRadius: "8px", padding: "60px", textAlign: "center", color: "#7f8c8d", border: "1px solid #ecf0f1" }}>
          <p style={{ fontSize: "15px", margin: 0 }}>Select a recruitment above to enter exam scores.</p>
        </div>
      ) : (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "16px", marginBottom: "24px" }}>
            {[
              { label: "Shortlisted Candidates", value: applications.length, color: "#3b82f6" },
              { label: "Scored", value: applications.filter(a => a.scored).length, color: "#10b981" },
            ].map((s, i) => (
            ...
            {loading ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead style={{ background: "linear-gradient(135deg, #f8fafc, #f1f5f9)" }}>
                  <tr>
                    {["Candidate", "Email", "Written Score", "Oral Score", "Total", "Action"].map(h => (
                ...
                <tbody>
                  {applications.map(row => {
                    ...
                    const written = parseFloat(s.written) || 0;
                    const oral = parseFloat(s.oral) || 0;
                    const total = s.written && s.oral ? Math.round((written + oral) / 2) : null;
                    return (
                      <tr key={row.id} style={{ borderTop: "1px solid #f9fafb" }}>
                        ...
                        <td style={{ padding: "14px 20px" }}>
                          <input type="number" min="0" max="100" placeholder="0-100" value={s.written || ""}
                            onChange={e => handleScoreChange(row.id, "written", e.target.value)}
                            style={{ width: "80px", padding: "6px 10px", border: "2px solid #e5e7eb", borderRadius: "8px", fontSize: "13px", outline: "none" }} />
                        </td>
                        <td style={{ padding: "14px 20px" }}>
                          <input type="number" min="0" max="100" placeholder="0-100" value={s.oral || ""}
                            onChange={e => handleScoreChange(row.id, "oral", e.target.value)}
                            style={{ width: "80px", padding: "6px 10px", border: "2px solid #e5e7eb", borderRadius: "8px", fontSize: "13px", outline: "none" }} />
                        </td>
                        <td style={{ padding: "14px 20px", fontWeight: "700", color: total !== null ? (total >= 60 ? "#10b981" : "#ef4444") : "#9ca3af" }}>
                          {total !== null ? total : "—"}
                        </td>
                        <td style={{ padding: "14px 20px" }}>
                          <button onClick={() => submitScore(row.id)} disabled={!s.written || !s.oral}
                            style={{ padding: "6px 16px", background: s.written && s.oral ? "linear-gradient(135deg, #3b82f6, #8b5cf6)" : "#e5e7eb", color: s.written && s.oral ? "white" : "#9ca3af", border: "none", borderRadius: "8px", cursor: s.written && s.oral ? "pointer" : "default", fontWeight: "600", fontSize: "12px" }}>
                            Save Score
                          </button>
                        </td>
                      </tr>
