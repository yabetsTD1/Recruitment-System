...
export default function ExperienceSection({
  ...
  const saveEntry = async (index) => {
    ...
    const entry = experience[index];
    if (!entry.jobTitle?.trim() || !entry.institution?.trim()) {
      setSaveMsg(p => ({ ...p, [index]: "error:Job Title and Institution are required" }));
      return;
    }
    setSavingIdx(index);
    try {
    ...
    <div style={{
        ...
        <div className="animate-in">
          ...
          {experience.length === 0 && (
            <div style={{ textAlign: "center", padding: "28px", background: "#f8fafc", borderRadius: "10px", border: "2px dashed #e2e8f0", marginBottom: "16px", color: "#94a3b8", fontSize: "14px" }}>
              No new experience entries. Click <strong>Add Experience</strong> to add one.
            </div>
          )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: "20px", padding: "0 24px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px" }}>
          ...
          <button type="button"
            onClick={() => {
              const hasUnsaved = experience.some(e => !e.id && (e.jobTitle?.trim() || e.institution?.trim()));
              if (hasUnsaved) { setNextError("Please save all entries before proceeding."); return; }
              if (savedRecords.length === 0 && experience.filter(e => e.id).length === 0) {
                setNextError("Please add and save at least one experience entry.");
                return;
              }
              setNextError("");
              goToNextStep();
