...
const BUDGET_YEARS = Array.from({ length: 10 }, (_, i) =>
  ...
  const handleOpen = () => {
    if (!open) {
      const rootIds = new Set(
        orgUnits.filter((u) => !u.parentId).map((u) => u.id),
      );
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        <span style={{ fontSize: "10px", color: "#7f8c8d", flexShrink: 0 }}>
          {open ? "▲" : "▼"}
        </span>
      </div>
      {open && (
        ...
        >
          ...
          {tree.length === 0 ? (
            <div
              style={{ padding: "12px", color: "#9ca3af", fontSize: "13px" }}
            >
              No org units found
            </div>
          ) : (
...
export default function RecruitmentRequestPage() {
    ...
    if (row.jobQualificationId) {
      try {
        const res = await api.get(
          `/admin/job-qualifications/${row.jobQualificationId}/entries`,
        );
        setQualEntries(res.data || []);
      } catch {
        setQualEntries([]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.jobQualificationId) {
      setMsg({ type: "error", text: "Please select a Required Job." });
      return;
    }
    if (!form.jobLocation) {
      setMsg({ type: "error", text: "Please select a Working Place." });
      return;
    }
    setSaving(true);
    try {
      const payload = {
        ...form,
        recorderName: user?.fullName || form.recorderName,
      };
      if (editId) {
        ...
        setMsg({
          type: "success",
          text: `Request submitted! Batch Code: ${res.data.batchCode}`,
        });
      }
      await loadAll();
      setShowForm(false);
    ...
    <div style={{ fontFamily: "sans-serif", color: "#2c3e50" }}>
      ...
      >
        <div>
          ...
          >
            Submit recruitment requests for approval
          </p>
        </div>
        <button
          onClick={() => {
            ...
            {loading ? (
              ...
              requests
                ...
                <tr key={row.id} style={{ borderTop: "1px solid #f0f3f4" }}>
                  ...
                  >
                    {row.employmentType || row.hiringType || "—"}
                  </td>
                  <td
                    style={{
                      padding: "11px 14px",
          ...
          >
            ...
            <form onSubmit={handleSubmit} style={{ padding: "24px 28px" }}>
              ...
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => set("incrementStep", e.target.value)}
                    style={inp}
                  >
                    <option value="">--Select One--</option>
                  ...
                  <tbody>
                    {qualEntries.length === 0 ? (
                      <tr>
                        ...
                        >
                          {form.jobQualificationId
                            ? "No qualification entries for this job."
                            : "Select a Required Job above to see qualification requirements."}
                        </td>
                      </tr>
                    ) : (
              ...
              >
                <div>
                  <label style={lbl}>
                    Required Number <span style={{ color: "#e74c3c" }}>*</span>
                  </label>
                  <input
                    required
                    type="number"
                    ...
                    onChange={(e) => set("vacancyNumber", e.target.value)}
                    style={inp}
                    placeholder="e.g. 2"
                  />
                </div>
              ...
              >
                <div>
                  <label style={lbl}>
                    Recruitment Type <span style={{ color: "#e74c3c" }}>*</span>
                  </label>
                  <select
                    required
                    value={form.recruitmentType}
                    onChange={(e) => set("recruitmentType", e.target.value)}
                    style={inp}
                  >
                    <option value="">--Select One--</option>
                ...
                <div>
                  ...
                  >
                    {form.batchCode || (
                      ...
                      >
                        Auto-generated when job is selected
                      </span>
                    )}
                  </div>
                </div>
              ...
              >
                <div>
                  ...
                  <input
                    value={user?.fullName || form.recorderName}
                    readOnly
                    style={{
                      ...inp,
                      background: "#f3f4f6",
