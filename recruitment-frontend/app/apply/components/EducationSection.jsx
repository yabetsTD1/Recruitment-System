...
export default function EducationSection({
  ...
  const deleteSaved = async (id) => {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/education/${id}`, { method: "DELETE" }).catch(() => {});
  ...
  const removeEducation = (index) => {
    ...
    if (entry.id) {
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/education/${entry.id}`, { method: "DELETE" }).catch(() => {});
    }
  ...
  const saveEntry = async (index) => {
    ...
    try {
      const entry = education[index];
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/applicant/education`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...entry, email }),
      });
      const data = await res.json();
      if (!res.ok) { setSaveMsg(p => ({ ...p, [index]: "error:" + (data.message || "Failed") })); return; }
      // Store returned id back into state
      const updated = [...education];
        ...
        <div className="animate-in">
          ...
          {savedRecords.length > 0 && (
            <div style={{ marginBottom: "24px", background: "white", borderRadius: "10px", overflow: "hidden", border: "2px solid #e2e8f0" }}>
              <div style={{ padding: "10px 16px", background: "linear-gradient(135deg, #8b5cf6, #7c3aed)", color: "white", fontSize: "13px", fontWeight: "700" }}>
                Saved Education Records ({savedRecords.length})
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead style={{ background: "#f8fafc" }}>
                  <tr>
                    {["#", "Institution", "Field of Study", "Level", "Start", "End", "Action"].map(h => (
                ...
                <tbody>
                  {savedRecords.map((r, i) => (
                    <tr key={r.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "9px 12px", fontSize: "13px", fontWeight: "600", color: "#1f2937" }}>{r.institution || "—"}</td>
                      <td style={{ padding: "9px 12px", fontSize: "13px", color: "#6b7280" }}>{r.fieldOfStudy || "—"}</td>
                      <td style={{ padding: "9px 12px", fontSize: "13px", color: "#6b7280" }}>{r.educationLevel || "—"}</td>
          ...
          {education.length === 0 && (
            <div style={{ textAlign: "center", padding: "28px", background: "#f8fafc", borderRadius: "10px", border: "2px dashed #e2e8f0", marginBottom: "16px", color: "#94a3b8", fontSize: "14px" }}>
              No new education entries. Click <strong>Add Entry</strong> to add one.
            </div>
          )}
            ...
            <div key={index} className="entry-card" style={{
              ...
              <div style={{ marginTop: "20px", textAlign: "center" }}>
                ...
                <button
                  ...
                  style={{
                    padding: "10px 28px",
                    background: savingIdx === index ? "#94a3b8" : "linear-gradient(135deg, #8b5cf6, #7c3aed)",
                    color: "white", border: "none", borderRadius: "8px",
                    cursor: savingIdx === index ? "not-allowed" : "pointer",
