...
export default function DocumentsSection({
  ...
  const fetchDocs = useCallback(async () => {
    try {
      const res = await fetch(
        `${API}/public/applicant/document?email=${encodeURIComponent(email)}`,
      );
  ...
  const uploadFilesForType = async (docType, fileList, inputEl) => {
    if (!email) {
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(`${API}/public/applicant/document`, {
          method: "POST",
          body: fd,
        });
        if (!res.ok) {
          const d = await res.json().catch(() => ({}));
          setMsg("error:" + (d.message || "Upload failed."));
        }
      }
      await fetchDocs();
      setMsg(`success:${docType} uploaded successfully.`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`${API}/public/applicant/document/${id}`, {
        method: "DELETE",
      });
    ...
    >
      ...
      >
        ...
        <span
          style={{
            ...
            fontSize: "24px",
            fontWeight: "bold",
            transform: expandedSections.documents
              ? "rotate(90deg)"
              : "rotate(0deg)",
      ...
      {expandedSections.documents && (
        <div className="animate-in">
          ...
          >
            <label
              style={{ ...labelStyle, fontSize: "14px", marginBottom: "12px" }}
            >
              <div
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {documentTypeOptions.map((docType) => {
                    const inputId = `fileUpload_${docType.replace(/\s+/g, "_")}`;
                    return (
                      <tr
                        key={docType}
