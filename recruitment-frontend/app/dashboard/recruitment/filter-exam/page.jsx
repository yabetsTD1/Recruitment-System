...
export default function FilterCandidatesExamPage() {
  ...
  const selectRec = (r) => {
    ...
    api.get(`/recruitments/${r.id}/applications`)
      .then(res => {
        setApplications(res.data.filter(a => a.status === "SUBMITTED" || a.status === "UNDER_REVIEW"));
        setLoading(false);
      })
  ...
  const statusStyle = { SUBMITTED: { background: "#fef3c7", color: "#92400e" }, UNDER_REVIEW: { background: "#dbeafe", color: "#1d4ed8" } };
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "24px" }}>
            {[
              ...
              { label: "Under Review", value: applications.filter(a => a.status === "UNDER_REVIEW").length, color: "#8b5cf6" },
            ].map((s, i) => (
          ...
          <div style={{ background: "white", borderRadius: "16px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #f3f4f6", overflow: "hidden" }}>
            {loading ? (
              ...
              <div style={{ padding: "40px", textAlign: "center", color: "#9ca3af" }}>No pending applications for this recruitment.</div>
            ) : (
